...
月度和年度趋势的时间轴从每个游戏的公测日期开始，一直生成到公开源最新月份；版本趋势内置开服至今的版本期次目录，使用两个下拉栏选择任意起止小版本，默认显示最近 4 个可估算期次，也可以一键选择开服至今。当前免费公开月流水源只覆盖最近 12 个月，早期月份和缺少可靠卡池日期的历史期次会显示为空且折线断开，代码不会用均值、插值或虚构角色补齐。以后接入更早的合法档案时，同一时间轴会自动填充对应空位。
